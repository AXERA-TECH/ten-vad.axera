...
static int find_tensor(const AX_ENGINE_IOMETA_T* tensors, AX_U32 count,
                       const char* name) {
  for (AX_U32 i = 0; i < count; ++i) {
    if (tensors[i].pName != NULL && strcmp(tensors[i].pName, name) == 0) {
      return (int)i;
    }
  }
  return -1;
...
AUP_MODULE_AIVAD::AUP_MODULE_AIVAD(const char* model_path) {
  ...
  if (model_file == NULL) {
    printf("Failed to open AXModel: %s\n", model_path);
    return;
  }
  ...
  if (model_data == NULL ||
      fread(model_data, 1, (size_t)model_size, model_file) !=
          (size_t)model_size) {
    fclose(model_file);
    return;
  }
  ...
  engine_inited = 1;
  if (AX_ENGINE_CreateHandle(&engine_handle, model_data, (AX_U32)model_size) !=
          0 ||
      AX_ENGINE_CreateContext(engine_handle) != 0 ||
      AX_ENGINE_GetIOInfo(engine_handle, &io_info) != 0) {
    printf("Failed to create AX Engine model/context\n");
    return;
  }
  if (io_info->nInputSize != AUP_AED_MODEL_IO_NUM ||
      io_info->nOutputSize != AUP_AED_MODEL_IO_NUM) {
    printf("Unexpected AXModel IO count: %u/%u\n", io_info->nInputSize,
           io_info->nOutputSize);
    return;
  }
  ...
  const AX_S8* token = (const AX_S8*)"ten-vad";
  for (AX_U32 i = 0; i < io.nInputSize; ++i) {
    io.pInputs[i].nSize = io_info->pInputs[i].nSize;
    if (AX_SYS_MemAlloc(&io.pInputs[i].phyAddr, &io.pInputs[i].pVirAddr,
                        io.pInputs[i].nSize, 128, token) != 0) {
      printf("Failed to allocate input %u\n", i);
      return;
    }
  }
  for (AX_U32 i = 0; i < io.nOutputSize; ++i) {
    io.pOutputs[i].nSize = io_info->pOutputs[i].nSize;
    if (AX_SYS_MemAlloc(&io.pOutputs[i].phyAddr, &io.pOutputs[i].pVirAddr,
                        io.pOutputs[i].nSize, 128, token) != 0) {
      printf("Failed to allocate output %u\n", i);
      return;
    }
  }
  inited = 1;
...
AUP_MODULE_AIVAD::~AUP_MODULE_AIVAD() {
  ...
  if (io.pOutputs != NULL) {
    for (AX_U32 i = 0; i < io.nOutputSize; ++i) {
      if (io.pOutputs[i].pVirAddr != NULL) {
        AX_SYS_MemFree(io.pOutputs[i].phyAddr, io.pOutputs[i].pVirAddr);
      }
    }
    delete[] io.pOutputs;
  }
  if (engine_handle != NULL) AX_ENGINE_DestroyHandle(engine_handle);
  if (engine_inited) AX_ENGINE_Deinit();
  if (sys_inited) AX_SYS_Deinit();
  free(model_data);
}

int AUP_MODULE_AIVAD::Process(float* input, float* output) {
  if (!inited) {
    printf("AX Engine backend not initialized\n");
    return -1;
  }
  ...
  const char* input_names[] = {"input_1", "input_2", "input_3", "input_6",
                               "input_7"};
  const void* input_data[] = {input_data_buf_0, input_data_buf_1234[0],
                              input_data_buf_1234[1], input_data_buf_1234[2],
                              input_data_buf_1234[3]};
  const size_t input_sizes[] = {
      sizeof(input_data_buf_0), sizeof(input_data_buf_1234[0]),
      sizeof(input_data_buf_1234[1]), sizeof(input_data_buf_1234[2]),
      sizeof(input_data_buf_1234[3])};
  for (int logical = 0; logical < AUP_AED_MODEL_IO_NUM; ++logical) {
    int index = find_tensor(io_info->pInputs, io_info->nInputSize,
                            input_names[logical]);
    if (index < 0 || io.pInputs[index].nSize != input_sizes[logical]) return -1;
    memcpy(io.pInputs[index].pVirAddr, input_data[logical],
           input_sizes[logical]);
  }
  if (AX_ENGINE_RunSync(engine_handle, &io) != 0) return -1;

  const char* output_names[] = {"output_1", "output_2", "output_3",
                                "output_6", "output_7"};
  for (int logical = 0; logical < AUP_AED_MODEL_IO_NUM; ++logical) {
    int index = find_tensor(io_info->pOutputs, io_info->nOutputSize,
                            output_names[logical]);
    if (index < 0) return -1;
    if (logical == 0) {
      if (io.pOutputs[index].nSize != sizeof(float)) return -1;
      memcpy(output, io.pOutputs[index].pVirAddr, sizeof(float));
    } else {
      if (io.pOutputs[index].nSize != sizeof(input_data_buf_1234[logical - 1]))
        return -1;
      memcpy(input_data_buf_1234[logical - 1], io.pOutputs[index].pVirAddr,
             sizeof(input_data_buf_1234[logical - 1]));
    }
  }
  return 0;
}
...
int AUP_Aed_memAllocate(void* stPtr, const Aed_StaticCfg* pCfg) {
  ...
  if (stHdl->aivadInf == NULL) {
    stHdl->aivadInf = new AUP_MODULE_AIVAD("axmodel/ten-vad-ax650.axmodel");
    if (stHdl->aivadInf == NULL) {
      return -1;
    }
  }
